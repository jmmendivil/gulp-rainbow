...
module.exports = function(options) {
  ...
  options.paths = expandVariables({
    ...
    styles: {
      ...
      autoprefixer: autoprefixerOptions || { browsers: [ '> 1%', 'last 2 versions' ] },
  ...
  // - add it to the end!
  if (util.env.notify) {
    gulp.task('nofity-ends', function(){
      return gulp.src('src/main/**/*.js')
      .pipe(notify({
        "title": "Don't panic!",
        "message" : "All Gulp/Cherry tasks done.",
        "icon": path.join(__dirname, "gex-icon.png"),
        "sound": "Frog",
        "onLast": true,
        "wait": false
      }));
    });
    main.push('nofity-ends');
  }
